...
\[  \min_{{\bf w}, {\bf \psi}} \; {\cal C},\; {\cal C} \deq {\cal L}({\bf w}) + \mu \, \sum_{n,n'} R_{nn'} \, \|{\bf \psi}_n -  {\bf \psi}_{n'}\|^2 + \sum_n \lambda_n \, ({\bf o}_n - {\bf \psi}_n) + \gamma \, \|{\bf o}_n - {\bf \psi}_n\|^2\]
...
The adjustment of the variational problem hyper-parameters can be performed following different heuristics:
\\ - Starts minimizing the criterion with $\mu = 0$ (i.e., without regularization) and then increase $\mu$ until the cross-validation improves. The drawback of this simple approach is that the optimization problem is highly nonlinear and nonconvex. We thus may get stuck in a local minima.
\\- Starts considering the manifold as the union of 0-dimensional manifolds, i.e., prototypes 
