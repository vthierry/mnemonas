...
sl_abc_ab := z(solve({p(-1) = p1, p(-2) = p2}, {beta, nu}));
...
eq_abc := eliminate(eq_abc_2, {beta, nu}):
eq_abc_zc := collect(op(1,op(2, eq_abc)), {cT, c}, e -> z(e), distributed);
zero := expand(subs(p1 = p(-1), p2 = p(-2), p3 = p(-3), gT = g^T, cT = g^T / c^T, c^T * (c-g) * eq_abc_zc)) assuming T::integer;

eq_abc_zc_0 := limit(limit(eq_abc_zc, cT = 0), gT = 0):
