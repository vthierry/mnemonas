...
/** Generates the experimental results of <a href="https://hal.inria.fr/hal-01610735v1">the preliminary research report</a> on backward-tuning. */
class Experimentation1 {
  std::string header;
...
public:
  Experimentation1() : input(NULL), output(NULL), transform(NULL), seed(0), verbose_but_nosave(false) {}
  ~Experimentation1() {
    delete input;
    delete output;
    delete transform;
  ...
  /** Encapsulates the exponential decay fit latex result table construction. */
  class ResultTable {
...
public:
  ...
  unsigned int run_sequence_generation(String name, String type, char criterion, unsigned int N, unsigned int T)
  {
    ...
    // Returns a string view of the transform a posteriori Hamming distance and related binary errors.
    {
      ...
      std::string filename = s_printf("tex/results/sequence_generation/sequence_errors_%s_criterion=%c_N=%d_T=%d.txt", type.c_str(), criterion, transform->N, T);
      // - printf("> %s\n", s.c_str());
      s_save(filename, s);
