...
std::string network::KernelObservableEstimator::Observable::asString() const
{
  return s_printf("{ 'name' = '%s', 'value' = '%g' }", name.c_str(), value);
}
double network::KernelObservableEstimator::Observable::getValue() const
{
  assume(input != NULL, "illegal-state", "in network::KernelObservableEstimator::Observable::getValueDerivative, attempt to call this method before the observable reset()");
  return value;
}
double network::KernelObservableEstimator::Observable::getValueDerivative(unsigned int n, double t) const
{
  assume(input != NULL, "illegal-state", "in network::KernelObservableEstimator::Observable::getValueDerivative, attempt to call this method before the observable reset()");
  return const_cast < Observable * > (this)->doValueDerivative(n, t);
}
double network::KernelObservableEstimator::Observable::doValue()
{
  assume(false, "illegal-state", "in network::KernelObservableEstimator::Observable::doValue, this virtual method must be overriden");
  return NAN;
}
double network::KernelObservableEstimator::Observable::doValueDerivative(unsigned int n, double t)
{
  assume(false, "illegal-state", "in network::KernelObservableEstimator::Observable::doValueDerivative, this virtual method must be overriden");
  return NAN;
}
network::KernelObservableEstimator::KernelObservableEstimator(KernelTransform& transform, std::vector < Observable * > observables_, const double *values_, const double *lambdas_) : network::KernelEstimator(transform)
{
...
network::KernelObservableEstimator::KernelObservableEstimator(KernelTransform& transform, const Input& input, std::vector < Observable * > observables_, const double *lambdas_) : KernelObservableEstimator(transform, observables_, NULL, lambdas_)
{
  for(unsigned int k = 0; k < dimension; k++)
...
double network::KernelObservableEstimator::drho(unsigned int n, double t) const
{
  double v = 0;
  for(unsigned int k = 0; k < dimension; k++) {
    double e = observables[k]->getValue() - values[k];
...
double network::KernelObservableEstimator::getObservableExpectedValue(unsigned int k) const
{
  assume(k < dimension, "illegal-argument", "in network::KernelObservableEstimator::getObservableExpectedValue the index k=%d must be in {0, %d{", k, dimension);
  return values[k];
}
network::KernelObservableEstimator::Observable *network::KernelObservableEstimator::getObservable(String name, ...)
{
  va_list a;
  va_start(a, name);
  if(name == "mean") {
...
protected:
    ...
    va_end(a);
    assume(false, "illegal-argument", "in network::KernelObservableEstimator::getObservable undefined predefined observable name '%s", name.c_str());
    Observable o("none");
    return NULL;
  }
}
std::vector < network::KernelObservableEstimator::Observable * > network::KernelObservableEstimator::getObservables(String name, ...) {
  va_list a;
  va_start(a, name);
  if(name == "mean") {
    unsigned int N = va_arg(a, unsigned int);
    assume(0 < N && N <= 32, "illegal-argument", "network::KernelObservableEstimator::getObservables the number of unit N=%d is numerically unrealistic, must be in {1, 32}", N);
    va_end(a);
    ...
    unsigned int N = va_arg(a, unsigned int);
    assume(0 < N && N <= 8, "illegal-argument", "network::KernelObservableEstimator::getObservables the number of unit N=%d is numerically unrealistic, must be in {1, 8}", N);
    va_end(a);
    std::vector < Observable * > observables;
    for(unsigned int n = 0; n < N; n++)
      observables.push_back(getObservable("mean", n));
    ...
    unsigned int N = va_arg(a, unsigned int);
    unsigned int tau = va_arg(a, unsigned int);
    assume(0 < N && N * tau <= 32, "illegal-argument", "network::KernelObservableEstimator::getObservables the number of unit N=%d and/or tau=%d is numerically unrealistic, must tau * N = %d be in {0, 32}, while 0 < N", N, tau);
    va_end(a);
    std::vector < Observable * > observables;
    for(unsigned int n = 0; n < N; n++)
      observables.push_back(getObservable("mean", n));
    ...
    va_end(a);
    assume(false, "illegal-argument", "in network::KernelObservableEstimator::getObservables undefined predefined observable name '%s", name.c_str());
    std::vector < Observable * > observables;
    return observables;
  }
}
void network::KernelObservableEstimator::deleteObservables(std::vector < network::KernelObservableEstimator::Observable * >& observables)
{
