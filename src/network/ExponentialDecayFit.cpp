...
network::ExponentialDecayFit::ExponentialDecayFit(const ExponentialDecayFit& fit) : gamma(fit.gamma), n(fit.n), c1(fit.c1), T0(fit.T0), T1(fit.T1), T2(fit.T2), L0(fit.L0), L1(fit.L1), cmin(fit.cmin), bias(fit.bias)
{
  values = fit.values;
}
network::ExponentialDecayFit::ExponentialDecayFit(unsigned int window) : gamma(window == 0 ? 1 : pow(0.1, 1.0 / window))
{
  clear();
}
void network::ExponentialDecayFit::clear()
{
  c1 = NAN, n = T0 = T1 = T2 = L0 = L1 = 0, cmin = NAN, bias = NAN;
  values.clear();
}
double network::ExponentialDecayFit::add(double c)
{
  double tau = getDecay();
  if((c < c1) && !std::isnan(c1)) {
    double d = c1 - c, l = log(d);
    T2 = gamma * (T0 + 2 * T1 + T2);
    T1 = gamma * (T0 + T1);
    T0 = 1 + gamma * T0;
    L1 = gamma * (L0 + L1);
...
double network::ExponentialDecayFit::getDecay() const
{
  return T0 * L1 > T1 * L0 ? (T0 * T2 - T1 * T1) / (T0 * L1 - T1 * L0) : NAN;
}
double network::ExponentialDecayFit::getBias() const
{
  double tau = getDecay();
