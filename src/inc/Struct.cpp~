...
void Struct::dump(String header) const
{
  ...
  printf("] values = { ");
  for(std::map < std::string, Struct * > ::const_iterator i = values.begin(); i != values.end(); i++)
    printf("'%s':«%s»@%ld ", i->first.c_str(), ((String) * i->second).c_str(), (long int) i->second);
  printf("}\n");
}
void Struct::clean()
{
  ...
  for(int i = 0; i < length; i++) {
    ...
    if(values.count(n) > 0) {
      Struct *s = values[n];
      assume(s != NULL, "illegal-Struct", "Spurious NULL value for index %d in '%s'\n", i, value.c_str());
      if(s->isEmpty())
        erase(n);
    }
  }
  ...
  for(std::vector < std::string > ::iterator i = names.begin(); i != names.end();) {
    ...
    Struct *s = values[n];
    assume(s != NULL, "illegal-Struct", "Spurious NULL value for name '%s' in '%s'\n", n.c_str(), value.c_str());
    if(s->isEmpty())
      erase(n);
    else
      i++;
...
Struct& Struct::get(String name, bool path) const
{
  if(path) {
    std::size_t found = name.find_first_of("/");
    if(found != std::string::npos)
      return get(name.substr(0, found)).get(name.substr(found + 1), true);
  }
  Struct *s = const_cast < Struct * > (this);
  s->insert(name, false);
  return *s->values[name];
}
Struct& Struct::set(String name, const Struct& value, bool path)
{
  if(path) {
    std::size_t found = name.find_first_of("/");
    if(found != std::string::npos)
      return get(name.substr(0, found)).set(name.substr(found + 1), value, true);
  }
  insert(name, true);
...
private:
    ...
    void write_value(std::string& string, const Struct& value)
    {
      if(value.isAtomic())
        ...
        string += asBeginTag("{");
        bool once = true;
        for(Struct::Iterator i(const_cast < Struct & > (value)); i.next();) {
          if(once)
            once = false;
          else
            string += asNextTag(", ");
