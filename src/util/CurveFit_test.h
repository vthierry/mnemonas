...
void CurveFit_test()
{
  ...
  fit.clear();
  for(double t = 0; t < 10000; t++)
    fit.add(1.2345 + 0.012345 * (t - 9999) + Density::gaussian(0, 0.02));
  ...
  fit.clear();
  for(double t = 0; t < 10000; t++)
    fit.add(2 * exp(-(t - 9999) / 3141.6) + 1.2345 + Density::gaussian(0, 0.0010));
  fit.show("/tmp/fit3.1", true);
  assume(fit.getMode() == 'e' && fabs(fit.getDecay() - 31416) < 1e-4 && fabs(fit.getBias() - 1.2345) < 1e-6, "illegal-state", "in CurveFit_test 3.1/3 wrong estimation : %s\n", fit.asString().c_str());
  fit.clear();
