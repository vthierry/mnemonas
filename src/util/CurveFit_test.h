...
void CurveFit_test()
{
  ...
  for(double t = 0; t < 100; t++)
    fit.add(1.2345);
  assume(fit.getMode() == 'c' && fabs(fit.getBias() - 1.2345) < 1e-10, "illegal-state", "in CurveFit_test 1/3 wrong estimation : %s\n", fit.asString().c_str());
  //fit.show("/tmp/fit1", true);
  fit.clear();
  for(double t = 0; t < 100; t++)
    fit.add(1.2345 + 0.012345 * (t - 99));
  assume(fit.getMode() == 'a' && fabs(fit.getGain() - 0.012345) < 1e-10 && fabs(fit.getBias() - 1.2345) < 1e-10, "illegal-state", "in CurveFit_test 2/3 wrong estimation : %s\n", fit.asString().c_str());
