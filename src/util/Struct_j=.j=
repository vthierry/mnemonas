...
text = The J= language aims at being a smart minimal specification language
 data-structure = Data structure
  ...
  examples = For instance:
   ...
   = The text is a data which is itself data-structure, the first section being entitled "Data structure" and indexed "data-structure" section as sub-structure
  = This section is separated from the next one by the following equal sign
 = More details on the syntax
  = A list is simply a data-structure with values indexed by non-negative integer, automatically incremented
  = To minimize typing :
   = The text label may be ommited as in the present section
   = The title label corresponds to 1st line of the data-structure
  = A string may contains additional syntaxic construct 
   ...
   = Formated sub-string using the "_" prefix and "_" suffix
    = A sub-string between "_", e.g., "_emphasized text_" corresponds to an emphasized text
     = The "_e_..._" class indicates an important part of the text (e for emphasized), it is the defaut, i.e. corresponds to simply using "_..._"
  ...
  = Discussion
   This syntax allows to define data-structure equivalent to a [https://www.json.org JSON] data-structure, but with a much simpler syntax, it also allows to have the expressive power of the [https://en.wikipedia.org/wiki/Markdown Markdown] syntax, allowing to _structure_ and _decorate_ an _hyper-text_. The idea comes from both the [https://en.wikipedia.org/wiki/YAML Yaml] syntax, here simplified, and the way code is structured in [https://en.wikipedia.org/wiki/Python_(programming_language) Python] by indentation. _q_Small is beautiful_.
